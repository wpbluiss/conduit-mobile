  ...
  return (
    <View style={[st.container, { backgroundColor: colors.bgPrimary, paddingTop: insets.top }]}>
      <View style={st.header}>
        <Text style={[st.headerTitle, { color: colors.textPrimary }]}>Calendar</Text>
      </View>
      <View style={st.emptyState}>
        <View style={st.iconWrap}>
          <Ionicons name="calendar-outline" size={48} color={Colors.electric} />
        </View>
        <Text style={[st.emptyTitle, { color: colors.textPrimary }]}>No data yet</Text>
        <Text style={[st.emptySubtitle, { color: colors.textSecondary }]}>
          Appointments and bookings captured by your AI agent will appear here.
        </Text>
      </View>
    </View>
  );
