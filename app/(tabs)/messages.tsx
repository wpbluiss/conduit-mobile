  ...
  return (
    <View style={[st.container, { backgroundColor: colors.bgPrimary, paddingTop: insets.top }]}>
      <View style={st.header}>
        <Text style={[st.headerTitle, { color: colors.textPrimary }]}>Messages</Text>
      </View>
      <View style={st.emptyState}>
        <View style={st.iconWrap}>
          <Ionicons name="chatbubbles-outline" size={48} color={Colors.electric} />
        </View>
        <Text style={[st.emptyTitle, { color: colors.textPrimary }]}>No messages yet</Text>
        <Text style={[st.emptySubtitle, { color: colors.textSecondary }]}>
          When your AI agent handles calls, SMS conversations with your leads will appear here.
        </Text>
      </View>
    </View>
  );
